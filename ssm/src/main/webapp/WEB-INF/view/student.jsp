...
<body class="gray-bg">
        ...
        <div class="row">
			  <div class="col-lg-2">
				<div class="input-group">
			      ...
			      <input type="date" name="sdbirth1" class="" id="sdbirth1" >
			      <input type="date" name="sdbirth2" class="" id="sdbirth2" >
				</div>
			  </div>
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
			      <span class="input-group-addon">一框多查 </span>
			      <input type="text" name="chaxun" class="form-control" id="chaxun" >
				</div>
			  </div>
			   -->
			...
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveUser.htm">
						<input type="hidden" name="sdId" id="hidden_txt_sdId" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								...
								<td>学生性别：</td>
								<td>
								<input type="radio" id="sdsex1" name="sdsex" value="男"/>男
								<input type="radio" id="sdsex2" name="sdsex" value="女"/>女
								</td>
							</tr>
							...
							<tr>
								<td>学生爱好：</td>
								<td>
								<input type="checkbox" id="sdHobby1" name="sdHobby" value="篮球"/>篮球
								<input type="checkbox" id="sdHobby2" name="sdHobby" value="足球"/>足球
								<input type="checkbox" id="sdHobby3" name="sdHobby" value="排球"/>排球
								</td>
								<td>&nbsp;&nbsp;</td>
								<td>学生生日：</td>
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_edit" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_import" enctype="multipart/form-date" method="post" action="importStudent.htm">
					
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>请选择：</td>
								<td><input type="file" id="studentFile" name="studentFile"
									class="form-control" aria-required="true" required/></td>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_import_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	<div class="modal fade" id="modal_user_count" role="dialog" aria-labelledby="modal_user_del" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					 <h4 class="modal-title" id="modal_user_del_head"> 柱状图  </h4>
				</div>
				<div class="modal-body">
					<div id="abc" style="height:500px; width: 300px"></div>
				</div>
				<div class="modal-footer">
				<button type="button" class="btn btn-danger"  id="del_user_btn">刪除</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
	...
	var TableInit = function () {
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            sdbirth1: $("#sdbirth1").val(),
	            sdbirth2: $("#sdbirth2").val(), 
	           /*  chaxun: $("#chaxun").val(), */
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_add").click(function(){
	    		...
	    		$("#form_user").resetForm();
	    		document.getElementById("hidden_txt_sdId").value='';
	    	...
	    	$("#btn_count").click(function(){
	    		$.ajax({
	    		    url:"countStudent.htm",
	    		    dataType:"json", 		 
	    		    type:"post",
	    		    success:function(res){
	    		    	if(res.success){
	    		    	//初始化echarts实例
	    		    	var myecharts=echarts.init(document.getElementById("abc"));
	    		    	
	    		    	var xdata=[];
	    		    	var ydata=[];
	    		    	res.data.forEach(function(zx){
						xdata.push(zx.mname);
						ydata.push({value: zx.num, name: zx.mname});
	...
	function initEditUser(getSelection){
		$('#hidden_txt_sdId').val(getSelection.sdId);
